...
// PEAR Token Contract configuration
const PEAR_TOKEN_ADDRESS = '0xc760a77356563560260c9E571AF6dbbab396bE38';
const MASTER_WALLET_ADDRESS = '0xAd23FFc8e1b16e771914aB9eED33eAF84d07cEd4';

// PEAR Token ABI (minimal)
const PEAR_TOKEN_ABI = [
...
const verifyAuthToken = async (req, res, next) => {
  ...
  try {
    ...
    const query = 'SELECT * FROM wallet_users WHERE auth_token = $1 AND is_active = true';
...
const tokenContract = new ethers.Contract(PEAR_TOKEN_ADDRESS, PEAR_TOKEN_ABI, masterWallet);

// Token balance endpoint
app.get('/api/tokens/balance', verifyAuthToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Could not fetch token balance',
      error: error.message 
    });
  }
});
...
app.post('/api/tokens/deduct', verifyAuthToken, async (req, res) => {
  try {
    const { walletAddress, amount = 100 } = req.body;
    
    // Validate input
    if (walletAddress.toLowerCase() !== req.user.wallet_address.toLowerCase()) {
    ...
    // Perform transfer from user to master wallet
    ...
    const remainingBalance = await tokenContract.balanceOf(walletAddress);

    res.json({
      success: true,
      remainingBalance: ethers.utils.formatUnits(remainingBalance, 18)
    });
  } catch (error) {
